...
export default class NodeHtmlParserExtractor implements PageDataExtractor.PageDataExtractorUtil {
  async extract(html: string) {
    const parsingDocument = await Utils.tryCatch(() => NodeHtmlParser.parse(html))

    if (parsingDocument.failed) return parsingDocument

    const document = parsingDocument.data
