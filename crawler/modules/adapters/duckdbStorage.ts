...
import * as ResultStoragePort from "../ports/resultStorage.js"
...
export class DuckdbStorageAdapter implements ResultStoragePort.ResultStorageOutput {
  async open(storageId: string): Promise<ResultStoragePort.ResultStore> {
    ...
    return {
      save: async (data: Entities.PageData) => {
        if (seenTxIds.has(data.txId)) return false
